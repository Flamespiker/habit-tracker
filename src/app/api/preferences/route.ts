...
import { getUserPreferences, saveUserPreferences } from '@/lib/db/mongo/user-preferences'
...
export async function PATCH(request: Request) {
  ...
  if (coaching_style !== undefined) {
    if (!VALID_COACHING_STYLES.includes(coaching_style as IUserPreferences['coaching_style'])) {
      return NextResponse.json({ error: 'Invalid coaching_style' }, { status: 400 })
    }
    update.coaching_style = coaching_style as IUserPreferences['coaching_style']
  }

  if (notification_time !== undefined) {
    if (typeof notification_time !== 'string' || !/^\d{2}:\d{2}$/.test(notification_time)) {
      return NextResponse.json({ error: 'Invalid notification_time — expected HH:MM' }, { status: 400 })
    }
