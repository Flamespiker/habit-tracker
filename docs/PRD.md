...
| `POST` | `/api/ai` ✅ | Generate a daily coaching nudge via Claude API (model from prompt YAML); fetches habits + checkins + preferences in parallel; saves to MongoDB via saveCoachingResponse(); returns `{ nudge: string }` | MongoDB (write), Supabase (read) |
...
| **5 — AI coaching** (in progress) | 9–10 | `@anthropic-ai/sdk` installed ✅, `POST /api/ai` (Claude API call, prompt loaded from YAML, saves to MongoDB) ✅, daily log "Log Day" button wired to POST /api/ai with loading state + nudge display ✅ — remaining: remove placeholder nudge write from checkins route, weekly summary agent, coaching history on goal detail pages | After clicking Log Day, you get a real AI coaching nudge. The core AI loop works end-to-end. |
