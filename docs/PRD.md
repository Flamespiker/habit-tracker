...
- Daily nudge: streamed via POST /api/ai (Vercel AI SDK v6 streamText + @ai-sdk/anthropic); surfaced on daily log and dashboard; guarded by rate limiter + input length + content filter (src/lib/ai/guardrails.ts)
...
| `POST` | `/api/coaching` ✅       | Save a coaching response; guardrails: rate limit (10/hr) + input length + content filter on `content` if string; body: `{ type, content, model, habit_context? }`; validates type enum; returns 201                             | MongoDB                          |
| `POST` | `/api/ai` ✅             | Stream a daily coaching nudge via Vercel AI SDK v6 `streamText` + `@ai-sdk/anthropic`; guardrails: rate limit (10/hr) + input length + content filter on `journal_entry`; fetches habits + checkins + preferences in parallel; `onFinish` saves completed nudge to MongoDB; returns `text/plain` streaming response | MongoDB (write), Supabase (read) |
...
| Method | Path             | What it does                                                                      | DB   |
| ------ | ---------------- | --------------------------------------------------------------------------------- | ---- |
| `GET`  | `/api/health` ✅ | Check Supabase + MongoDB connectivity; returns `{ status, supabase, mongodb, timestamp }`; excluded from middleware (public endpoint) | Both |
