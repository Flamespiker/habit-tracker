...
| **6 — Tests & CI/CD** | 11–12 | Playwright test suite: auth (login, redirect, invalid credentials) ✅, habits (create, cancel, toggle) ✅, goals (create, navigate to detail) ✅, checkins (dashboard + log page toggle, persistence after reload) ✅ — playwright.config.ts: 60s timeout, 1 worker, chromium only ✅ — GitHub Actions pipeline, Claude PR review agent | Every push runs tests automatically. PRs get AI review. Broken builds are caught before merge. |
