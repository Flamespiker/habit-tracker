...
| **6 — Tests & CI/CD**        | 11–12 | Playwright test suite: auth ✅, habits ✅, goals ✅, checkins ✅, connectivity ✅ — playwright.config.ts: 120s timeout, webServer builds + starts production server in CI ✅ — .github/workflows/ci.yml: Node 20, Next.js build cache, Playwright chromium, 8 secrets, artifact upload on failure ✅ — .github/workflows/claude-pr-review.yml: `custom_instructions` input (not `review_instructions`), `id-token: write` permission, reviews TS, DB usage, RLS, tests, conventions ✅ — .github/workflows/trunk.yml + .trunk/trunk.yaml: eslint, prettier, checkov, trivy ✅ — CI auth timeout fix: `fetchWithTimeout` (30s AbortController) on `global.fetch` in Supabase server client so `signInWithPassword` fails fast instead of hanging; explicit `timeout: 60000` on all auth `waitForURL` calls so `retries: 1` fits within the 120s test budget ✅                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Every push runs tests automatically. PRs get AI review. Broken builds are caught before merge.                                                                                         |
