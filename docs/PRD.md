...
| **5 — AI coaching** (in progress) | 9–10 | `@anthropic-ai/sdk` + Vercel AI SDK v6 (`ai`, `@ai-sdk/anthropic`) installed ✅, `POST /api/ai` streams via `streamText` + `toTextStreamResponse()` ✅, `onFinish` saves to MongoDB ✅, daily log "Log Day" button streams via fetch + ReadableStream + flushSync + rAF ✅ — remaining: remove placeholder nudge write from checkins route, weekly summary agent, coaching history on goal detail pages | After clicking Log Day, a real AI coaching nudge is generated and displayed. The core AI loop works end-to-end. |
