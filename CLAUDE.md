...
src/app/goals/ → goals list — async Server Component fetching real data; delegates list state to GoalsClient + /[id]/page.tsx implemented (title, status badge, target date, linked habits with category badge + streak; fetches getGoalById first — notFound() if missing or wrong user — then getGoalHabitIds sequentially, then getHabits + getCheckinsForPeriod in parallel if linked habits exist)
...
src/app/api/coaching/route.ts → GET /api/coaching (returns coaching history from MongoDB via getCoachingHistory(); accepts optional ?limit=N param, default 20, clamped 1–100); POST /api/coaching (auth-gated; guardrails: checkRateLimit → checkInputLength + checkContentFilter on content if string; saves a new coaching response via saveCoachingResponse(); body: { type, content, model, habit_context? }; validates type enum + required fields; returns 201)
src/app/api/internal/coaching/route.ts → POST /api/internal/coaching (no Supabase auth; authenticated via x-internal-secret header matching INTERNAL_API_SECRET env var; user_id read from INTERNAL_USER_ID env var; same body + validation as /api/coaching; returns 201 — used by automation/n8n workflows)
src/app/api/internal/weekly-summary/route.ts → POST /api/internal/weekly-summary (no Supabase auth; authenticated via x-internal-secret header matching INTERNAL_API_SECRET env var; user_id read from INTERNAL_USER_ID env var; calls runWeeklySummaryAgent(userId); returns saved document with 201 — used by automation/n8n workflows)
src/app/api/ai/route.ts → POST /api/ai (auth-gated; guardrails: checkRateLimit then checkInputLength + checkContentFilter on journal_entry if provided; fetches habits + today's checkins (Supabase) + user preferences (MongoDB) in parallel; loads daily-coaching-nudge prompt via loadPrompt(); streams response via Vercel AI SDK v6 streamText() + @ai-sdk/anthropic provider; onFinish callback saves completed nudge to MongoDB via saveCoachingResponse(); returns toTextStreamResponse() — text/plain streaming response)
...
src/lib/db/supabase/ → Supabase query functions: client.ts (browser), server.ts (SSR — `createClient()` wraps all fetch calls with a 30s AbortController timeout so auth Server Actions fail fast in CI instead of hanging until the Playwright test timeout), admin.ts (service role — bypasses RLS; used by /api/health, weekly-summary-agent.ts, and support-agent.ts), habits.ts (getHabits, getHabitById, createHabit, updateHabit(userId,id,updates,client?), deleteHabit(userId,id,client?), toHabit), goals.ts (getGoals, getGoalById, createGoal, updateGoal(userId,id,updates,client?), createGoalHabits, getGoalHabitIds, toGoal), checkins.ts (getTodayCheckins, getCheckins(userId,habitId,...), createCheckin, upsertCheckin, getCheckinsForPeriod, updateCheckin(userId,id,updates,client?))
...
src/lib/ai/guardrails.ts → three exported functions: checkInputLength(input) — rejects inputs > 2000 chars (400); checkContentFilter(input) — rejects common prompt-injection phrases case-insensitively (400); checkRateLimit(userId) — sliding-window rate limiter, max 10 AI requests per user per hour using module-level Map<string, number[]> (429); all return GuardrailResult ({ ok: true } | { ok: false; error; status }); applied to POST /api/ai and POST /api/coaching
src/lib/ai/weekly-summary-agent.ts → LangGraph ReAct agent; exported runWeeklySummaryAgent(userId): calls readHabitData tool (getHabits + getCheckinsForPeriod, 7 days via createAdminClient — no cookie auth required) and readCoachingHistory tool (getCoachingHistory, limit 7) in parallel via agent graph, generates structured WeeklySummaryContent JSON (regex-extracted from response to handle any preamble), saves to MongoDB as type: 'weekly_summary' via saveCoachingResponse(); must be called from a server-side context
src/lib/ai/support-agent.ts → LangGraph ReAct agent; exported runSupportAgent(userId, question): three tools — getAppHelp(topic) returns static help text for habits/goals/checkins/streaks/log/coaching/settings, getUserHabits fetches user's habits + 7-day checkins via createAdminClient() (no cookie auth required), getCoachingHistory fetches 5 most recent coaching entries from MongoDB; agent selectively calls tools based on the question; returns plain text answer string; must be called server-side
...
src/middleware.ts → Next.js middleware; refreshes Supabase session on every request (keeps auth tokens alive); redirects unauthenticated users to /login; redirects authenticated users away from /login and /signup; matcher excludes _next/static, _next/image, favicon.ico, api/health, api/internal, and static asset extensions
...
- **beforeEach pattern**: authenticated spec files (habits, goals, checkin) call `page.goto('/route')` + `page.waitForLoadState('networkidle')` in `beforeEach` — the networkidle wait is required because page.goto() resolves at the load event, before the Server Component's async Supabase fetch completes and the Client Component hydrates
- **afterAll teardown pattern**: use `createClient(url, anonKey)` + `signInWithPassword` to delete test rows directly from Supabase. Do NOT call `client.auth.signOut()` — Supabase JS v2 `signOut()` defaults to `scope: 'global'` which revokes all active sessions for the user, including the tokens stored in `.auth/user.json`, breaking subsequent spec files in the same run.
...
- **Dialog close after submit**: `page.waitForSelector('[role=dialog]', { state: 'hidden', timeout: 30000 })` — not `expect(dialog).not.toBeVisible()` — gives API call + animation time to complete
- **Server Component list after creation**: `waitForLoadState('networkidle')` → `page.goto(url)` → `waitForLoadState('networkidle')` before asserting new items — Server Components require a full re-navigation to re-fetch from DB. Do NOT use `page.reload()` — it throws `net::ERR_ABORTED` when Next.js streaming SSR is in progress (the stream gets aborted mid-flight)
...
- **networkidle in API-focused beforeEach**: do NOT use `waitForLoadState('networkidle')` when the only purpose of beforeEach is to seed auth cookies before a `page.request` API call. The dashboard keeps MongoDB and streaming SSR connections open, causing networkidle to hang for minutes and exhaust the test budget. Use `page.goto('/', { waitUntil: 'domcontentloaded' })` instead — cookies are associated with the origin as soon as the first response arrives
- **sign-out test is extracted to `z-signout.spec.ts`**: the sign-out test calls `supabase.auth.signOut()` with `scope: 'global'`, revoking the session server-side and invalidating `.auth/user.json` for the rest of the run. It lives in `z-signout.spec.ts` (sorts last alphabetically) so it runs after all other spec files
...
- **Authenticated API requests in tests**: use `page.request.post/get()` — NOT the standalone `request` fixture. The `request` fixture creates an isolated `APIRequestContext` that does not forward the browser's auth cookies, so middleware redirects the request to `/login` before the route body is read. `page.request` shares the browser context's cookie jar.
- **sign-out test ordering**: `signOut()` calls `supabase.auth.signOut()` with default `scope: 'global'`, which revokes the token server-side and invalidates `.auth/user.json` for the rest of the run. The sign-out test must be the last spec to execute — if needed, extract it to `z-signout.spec.ts` so it runs after all other files alphabetically.
